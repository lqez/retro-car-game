...
import {
  ...
  EXPLOSION_FIRE_COLORS, EXPLOSION_SMOKE_COLOR, EXPLOSION_BURST,
} from './constants.js';
...
export function explode(x, z) {
  ...
  for (let i = 0; i < burst.sparkCount; i++) {
    const idx = pHead % PCOUNT; pHead++;
    const p = pData[idx], m = pebbleMeshes[idx];
    const a = Math.random() * Math.PI * 2, sp = burst.sparkSpeedBase + Math.random() * burst.sparkSpeedRandom;
    p.active = true;
    ...
    m.scale.setScalar(burst.sparkScale);
